...
def _project_guide() -> str:
    lines = [
        "batchor is a durable OpenAI, Anthropic, and Gemini Batch runner with typed Pydantic outputs, resumable runs, durable artifacts, and a narrow operator CLI.",
        "",
        ...
        f"- Anthropic batching: {_repo_path('docs/design_docs/ANTHROPIC_BATCHING.md')}",
...
VALIDATION_GUIDES = {
    ...
    "provider": [
        "Use when changing OpenAI, Anthropic, or Gemini request shaping, batching, file handling, or provider normalization.",
        "Commands:",
        "- uv run ty check src",
        "- uv run pytest -q",
        "- uv run pytest tests/unit/test_batchor_openai_provider.py tests/unit/test_batchor_anthropic_provider.py tests/unit/test_batchor_gemini_provider.py tests/unit/test_batchor_tokens.py --no-cov -q",
    ],
    "storage": [
...
DOC_TOPICS = {
    "all": [
        ...
        (
            "Anthropic batching",
            "docs/design_docs/ANTHROPIC_BATCHING.md",
            "Message Batches request and result behavior.",
        ),
    ...
    "provider": [
        ...
        (
            "Anthropic batching",
            "docs/design_docs/ANTHROPIC_BATCHING.md",
            "Message Batches request shaping, result streaming, and provider limits.",
        ),
